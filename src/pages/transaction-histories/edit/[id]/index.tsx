...
import { useRoqClient, useTransactionHistoryFindFirst } from 'lib/roq';
...
function TransactionHistoryEditPage() {
  ...
  const { data, error, isLoading, mutate } = useTransactionHistoryFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: TransactionHistoryInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.transaction_history.update({
        data: values as RoqTypes.transaction_history,
        where: {
          id,
        },
      });
