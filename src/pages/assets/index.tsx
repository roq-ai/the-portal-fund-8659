...
import { useRoqClient, useAssetFindManyWithCount } from 'lib/roq';
...
export function AssetListPage(props: AssetListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useAssetFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.asset.delete({ where: { id } });
      await mutate();
    } catch (error) {
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/assets/view/${record.id}`} passHref legacyBehavior>
                <Button
