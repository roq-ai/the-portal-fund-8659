...
import { useRoqClient, useAssetFindFirst } from 'lib/roq';
...
function AssetEditPage() {
  ...
  const { data, error, isLoading, mutate } = useAssetFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: AssetInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.asset.update({
        data: values as RoqTypes.asset,
        where: {
          id,
        },
      });
