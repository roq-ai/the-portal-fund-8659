...
import { useRoqClient, useWalletFindFirst } from 'lib/roq';
...
function WalletEditPage() {
  ...
  const { data, error, isLoading, mutate } = useWalletFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: WalletInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.wallet.update({
        data: values as RoqTypes.wallet,
        where: {
          id,
        },
      });
