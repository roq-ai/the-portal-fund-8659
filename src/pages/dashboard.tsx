...
import { useRoqPlatformUserProfiles, useAssetCount, useBalanceCount, useTransactionHistoryCount } from 'lib/roq';
...
function DashboardPage() {
  ...
  const assetStat = useAssetCount();

  const balanceStat = useBalanceCount();

  const transactionHistoryStat = useTransactionHistoryCount();

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Assets',
      result: assetStat?.data,
      isLoading: assetStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Balances',
      result: balanceStat?.data,
      isLoading: balanceStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Transaction Histories',
      result: transactionHistoryStat?.data,
