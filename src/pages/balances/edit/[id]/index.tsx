...
import { useRoqClient, useBalanceFindFirst } from 'lib/roq';
...
function BalanceEditPage() {
  ...
  const { data, error, isLoading, mutate } = useBalanceFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: BalanceInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.balance.update({
        data: values as RoqTypes.balance,
        where: {
          id,
        },
      });
